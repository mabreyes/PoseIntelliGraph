...
def _get_project_intro_doc():
    project_intro_doc = []
    with open(
        osp.join("..", "..", "projects", "README.md"), "r", encoding="utf-8"
    ) as f:
        for line in f.readlines():
            ...
            if "example_project" in line:
                line = line.replace(
                    "./",
                    "https://github.com/open-mmlab/mmpose/" "tree/dev-1.x/projects/",
                )
...
def main():
    ...
    with open(
        osp.join("projects", "community_projects.md"), "w", encoding="utf-8"
    ) as f:
