...
def _parse_model_doc_path(path):
    ...
    task = titlecase(_rel_path[1].replace("_", " "), callback=_titlecase_callback)
...
def main():
    ...
    for task, dataset_dict in model_docs.items():
        ...
        for dataset, keywords_dict in dataset_dict.items():
            lines += ["<hr/>", "<br/><br/>", "", f"## {titlecase(dataset)} Dataset", ""]

            for keywords, doc in keywords_dict.items():
                keyword_strs = [titlecase(x.replace("_", " ")) for x in keywords]
                ...
                lines += [
                    "<br/>",
                    "",
                    (f'### {" + ".join(keyword_strs)}' f" on {dataset_str}"),
                    "",
                    doc["content"],
                    "",
                ]
