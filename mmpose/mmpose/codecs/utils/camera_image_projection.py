...
def camera_to_image_coord(
    root_index: int, kpts_3d_cam: np.ndarray, camera_param: Dict
    ...
    fx, fy = camera_param["f"] / 1000.0
    cx, cy = camera_param["c"] / 1000.0
    ...
    kpts_3d_image[..., :2] = camera_to_pixel(kpts_3d_cam.copy(), fx, fy, cx, cy)
...
def camera_to_pixel(
    kpts_3d: np.ndarray, fx: float, fy: float, cx: float, cy: float, shift: bool = False
) -> np.ndarray:
...
def pixel_to_camera(
    kpts_3d: np.ndarray, fx: float, fy: float, cx: float, cy: float
) -> np.ndarray:
    """Project keypoints from camera space to image space.
