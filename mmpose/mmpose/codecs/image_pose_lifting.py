...
class ImagePoseLifting(BaseKeypointCodec):
    ...
    def __init__(
        ...
        if keypoints_mean is not None:
            ...
            keypoints_mean = np.array(keypoints_mean, dtype=np.float32).reshape(
                1, num_keypoints, -1
            )
            keypoints_std = np.array(keypoints_std, dtype=np.float32).reshape(
                1, num_keypoints, -1
            )
        ...
        if target_mean is not None:
            ...
            target_mean = np.array(target_mean, dtype=np.float32).reshape(
                1, target_dim, -1
            )
            target_std = np.array(target_std, dtype=np.float32).reshape(
                1, target_dim, -1
            )
    ...
    def encode(
        ...
        if self.keypoints_mean is not None:
            ...
            keypoint_labels = (
                keypoint_labels - self.keypoints_mean
            ) / self.keypoints_std
        if self.target_mean is not None:
            ...
            lifting_target_label = (
                lifting_target_label - self.target_mean
            ) / self.target_std
    ...
    def decode(
        self, encoded: np.ndarray, target_root: Optional[np.ndarray] = None
