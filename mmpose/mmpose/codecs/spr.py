...
class SPR(BaseKeypointCodec):
    ...
    def __init__(
        ...
        if sigma is None:
            ...
            if generate_keypoint_heatmaps:
                assert len(sigma) == 2, (
                    "sigma for keypoints must be given "
                    "if `generate_keypoint_heatmaps` "
                    "is True. e.g. sigma=(4, 2)"
                )
            self.sigma = sigma

    def _get_heatmap_weights(
        self, heatmaps, fg_weight: float = 1, bg_weight: float = 0
    ):
    ...
    def encode(
        ...
        roots, roots_visible = get_instance_root(
            _keypoints, keypoints_visible, self.root_type
        )
    ...
    def decode(
        self, heatmaps: Tensor, displacements: Tensor
        ...
        root_heatmap_peaks = batch_heatmap_nms(
            heatmaps[None, -1:], self.decode_nms_kernel
        )
    ...
    def get_keypoint_scores(self, heatmaps: Tensor, keypoints: Tensor):
        ...
        keypoint_scores = (
            torch.nn.functional.grid_sample(
                heatmaps.unsqueeze(1), keypoints, padding_mode="border"
            )
            .view(k, -1)
            .transpose(0, 1)
            .contiguous()
