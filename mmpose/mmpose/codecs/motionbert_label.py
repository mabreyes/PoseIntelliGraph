...
class MotionBERTLabel(BaseKeypointCodec):
    ...
    def encode(
        ...
        assert (
            "w" in _camera_param and "h" in _camera_param
        ...
        lifting_target_label[..., :, :] = (
            lifting_target_label[..., :, :]
            - lifting_target_label[..., self.root_index : self.root_index + 1, :]
        )
    ...
    def decode(
        ...
        if w is not None and w.size > 0:
            assert w.shape == h.shape, (
                f"w and h should have the same shape, "
            ...
            assert w.ndim in {1, 2}, (
                f"w and h should have 1 or 2 dimensions, " f"but got {w.ndim}."
            )
            if w.ndim == 1:
                w = w[:, None]
                h = h[:, None]
        ...
        keypoints[..., :, :] = (
            keypoints[..., :, :]
            - keypoints[..., self.root_index : self.root_index + 1, :]
        )
        keypoints /= 1000.0
