...
class AssociativeEmbedding(BaseKeypointCodec):
    ...
    def _get_batch_topk(self, batch_heatmaps: Tensor, batch_tags: Tensor, k: int):
        ...
        topk_locs = torch.stack(
            [topk_indices % W, topk_indices // W], dim=-1
        )  # (B, K, TopK, 2)

        return topk_vals, topk_tags, topk_locs

    def _group_keypoints(
        self, batch_vals: np.ndarray, batch_tags: np.ndarray, batch_locs: np.ndarray
    ):
    ...
    def batch_decode(
        self, batch_heatmaps: Tensor, batch_tags: Tensor
        ...
        batch_groups = self._group_keypoints(
            batch_topk_vals, batch_topk_tags, batch_topk_locs
        )
