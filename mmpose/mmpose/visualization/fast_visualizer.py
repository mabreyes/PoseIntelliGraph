...
class FastVisualizer:
    ...
    def __init__(
        ...
        self.keypoint_colors = metainfo.get("keypoint_colors", [(255, 255, 255)])
    ...
    def draw_pose(self, img: np.ndarray, instances: Instances):
        ...
        for kpts, score in zip(keypoints, scores):
            ...
            for kid, kpt in enumerate(kpts):
                ...
                cv2.circle(
                    img, (int(x_coord), int(y_coord)), self.radius, (255, 255, 255)
                )

    def draw_points(
        self, img: np.ndarray, instances: Union[Instances, Dict, np.ndarray]
    ):
        ...
        if hasattr(instances, "keypoints"):
            ...
            if len(shape) == 2:
                ...
                raise ValueError(
                    "input keypoints should be in shape of"
                    "(Instance_num, Point_num, Point_dim)"
                )
        else:
            raise ValueError(
                "The keypoints should be:"
