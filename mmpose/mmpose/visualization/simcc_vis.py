...
class SimCCVisualizer:
    def draw_instance_xy_heatmap(
        ...
        if mix:
            map2d = cv.addWeighted(overlaid_image, 1 - weight, map2d, weight, 0)
        self.image_cover(
            white, map2d, int(blank_size[1] * 0.1), int(blank_size[0] * 0.1)
        )
    ...
    def creat_blank(self, size: Union[list, tuple], K: int = 20, interval: int = 10):
        """Create the background."""
        blank_height = int(max(size[0] * 2, size[0] * 1.1 + (K + 1) * (15 + interval)))
        blank_width = int(max(size[1] * 2, size[1] * 1.1 + (K + 1) * (15 + interval)))
    ...
    def image_cover(
        self, background: np.ndarray, foreground: np.ndarray, x: int, y: int
    ):
    ...
    def add_1d_heatmaps(
        ...
        y_startpoint, x_startpoint = [
            int(1.1 * map2d_size[1]),
        ...
        for i in range(K):
            ...
            y_startpoint[0] += add
        return background[
            : x_startpoint[1] + y_startpoint[1] + 1,
            : y_startpoint[0] + x_startpoint[0] + 1,
        ]
