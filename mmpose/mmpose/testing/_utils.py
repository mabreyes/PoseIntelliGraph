...
def get_coco_sample(
    ...
    if random_keypoints_visible:
        keypoints_visible = np.random.randint(0, 2, (num_instances, 17)).astype(
            np.float32
        )
    else:
...
def get_packed_inputs(
    ...
    for idx in range(batch_size):
        ...
        keypoints_visible = np.ones((num_instances, num_keypoints), dtype=np.float32)
        ...
        if num_levels > 1:
            keypoint_weights = np.tile(keypoints_visible[:, None], (1, num_levels, 1))
        else:
        ...
        if with_reg_label:
            gt_instance_labels.keypoint_labels = torch.FloatTensor(
...
def _rand_keypoints(rng, bboxes, num_keypoints):
    ...
    keypoints = (
        relative_pos * bboxes[:, None, :2] + (1 - relative_pos) * bboxes[:, None, 2:4]
    )
