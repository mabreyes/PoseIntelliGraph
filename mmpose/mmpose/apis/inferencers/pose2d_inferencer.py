...
class Pose2DInferencer(BaseMMPoseInferencer):
    ...
    def update_model_visualizer_settings(
        self, draw_heatmap: bool = False, skeleton_style: str = "mmpose", **kwargs
    ) -> None:
        ...
        if skeleton_style not in ["mmpose", "openpose"]:
            raise ValueError(
                "`skeleton_style` must be either 'mmpose' " "or 'openpose'"
            )

        if skeleton_style == "openpose":
            self.visualizer.set_dataset_meta(self.model.dataset_meta, skeleton_style)
