...
class BaseMMPoseInferencer(BaseInferencer):
    ...
    def __init__(
        ...
        super().__init__(model, weights, device, scope, show_progress=show_progress)
    ...
    def _init_detector(
        ...
        object_type = (
            DATASETS.get(self.cfg.dataset_type)
            .__module__.split("datasets.")[-1]
            .split(".")[0]
            .lower()
        )
    ...
    def _load_weights_to_model(
        ...
        if checkpoint is not None:
            ...
            if "dataset_meta" in checkpoint_meta:
                ...
                model.dataset_meta = dataset_meta_from_config(cfg, dataset_mode="train")
        else:
            print_log(
        ...
        if isinstance(inputs, str):
            ...
            if hasattr(backend, "isdir") and isdir(inputs):
                ...
                for filepath in filepath_list:
                    input_type = mimetypes.guess_type(filepath)[0].split("/")[0]
                    if input_type == "image":
                ...
                if input_type == "video":
                    ...
                    raise ValueError(
                        f"Expected input to be an image, video, "
    ...
    def _get_webcam_inputs(self, inputs: str) -> Generator:
        ...
        inputs = inputs.lower()
        assert inputs.startswith("webcam"), (
            f"Expected input to start with " f'"webcam", but got "{inputs}"'
        )
        ...
        **kwargs,
        ...
        if not self._video_input and len(inputs) > 0:
            ...
            if vis_out_dir is not None:
                ...
                assert not file_extension, (
                    f"the argument `vis_out_dir` "
        ...
        if pred_out_dir != "":
            for pred, data_sample in zip(result_dict["predictions"], preds):
                if self._video_input:
                    ...
                    fname = (
                        os.path.splitext(
                            os.path.basename(data_sample.metainfo["img_path"])
                        )[0]
                        + ".json"
                    )
                    mmengine.dump(pred, join_path(pred_out_dir, fname), indent="  ")
    ...
    def _finalize_video_processing(
        ...
        if pred_out_dir:
            fname = (
                os.path.splitext(os.path.basename(self.video_info["name"]))[0] + ".json"
            )
            predictions = [
            ...
            mmengine.dump(predictions, join_path(pred_out_dir, fname), indent="  ")
