...
class Pose3DInferencer(BaseMMPoseInferencer):
    ...
    def __init__(
        ...
        self.pose2d_model = Pose2DInferencer(
            pose2d_model if pose2d_model else "human",
            pose2d_weights,
            device,
            scope,
            det_model,
        ...
        self._keypoint_converter = partial(
            convert_keypoint_definition,
            pose_det_dataset=self.pose2d_model.model.dataset_meta["dataset_name"],
    ...
    def preprocess_single(
        ...
        pose_sequences_2d = collate_pose_sequence(
            pose_results_2d_copy, True, target_idx
        )
        ...
        for i, pose_seq in enumerate(pose_sequences_2d):
            ...
            keypoints_2d = (
                np.squeeze(keypoints_2d, axis=0)
    ...
    @torch.no_grad()
    def forward(
        self, inputs: Union[dict, tuple], disable_rebase_keypoint: bool = False
    ):
        ...
        for idx, pose_lift_res in enumerate(pose_lift_results):
            ...
            if keypoint_scores.ndim == 3:
                pose_lift_results[idx].pred_instances.keypoint_scores = np.squeeze(
    ...
    def visualize(
        ...
        for single_input, pred in zip(inputs, preds):
            ...
            visualization = self.visualizer.add_datasample(
                ...
                show=show,
                wait_time=wait_time,
                dataset_2d=self.pose2d_model.model.dataset_meta["dataset_name"],
            ...
            if vis_out_dir:
                img_name = (
                    os.path.basename(pred.metainfo["img_path"])
                    if "img_path" in pred.metainfo
                    else None
                )
