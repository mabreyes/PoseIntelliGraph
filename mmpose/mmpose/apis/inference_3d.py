...
def convert_keypoint_definition(keypoints, pose_det_dataset, pose_lift_dataset):
    ...
    """
    assert pose_lift_dataset in ["h36m", "h3wb"], (
        "`pose_lift_dataset` should be " f"`h36m`, but got {pose_lift_dataset}."
    )

    keypoints_new = np.zeros(
        (keypoints.shape[0], 17, keypoints.shape[2]), dtype=keypoints.dtype
    )
...
def extract_pose_sequence(pose_results, frame_idx, causal, seq_len, step=1):
    ...
    start = max(frame_idx % step, frame_idx - frames_left * step)
        ...
        num_frames - (num_frames - 1 - frame_idx) % step,
        frame_idx + frames_right * step + 1,
    )
    pose_results_seq = (
        [pose_results[0]] * pad_left
        + pose_results[start:end:step]
        + [pose_results[-1]] * pad_right
    )
    return pose_results_seq


def collate_pose_sequence(pose_results_2d, with_track_id=True, target_frame=-1):
    ...
    N = len(pose_results_2d[target_frame])  # use identities in the target frame
    if N == 0:
        return []
    ...
    for idx in range(N):
        ...
        pred_instances = pose_results_2d[target_frame][idx].pred_instances.clone()
        ...
        if not with_track_id:
            pose_seq.pred_instances.keypoints = np.stack(
                [frame[idx].pred_instances.keypoints for frame in pose_results_2d],
                axis=1,
            )
        else:
...
def inference_pose_lifter_model(
    model, pose_results_2d, with_track_id=True, image_size=None, norm_pose_2d=False
):
                ...
                for pose_res in pose_results_2d:
                    for data_sample in pose_res:
                        for bbox in data_sample.pred_instances.bboxes:
                            ...
                            bbox_scale += max(bbox[2] - bbox[0], bbox[3] - bbox[1])
                            num_bbox += 1
        ...
        for j, data_sample in enumerate(pose_res):
            ...
            for k in range(len(kpts)):
                ...
                if norm_pose_2d:
                    ...
                    keypoints.append(
                        (kpt[:, :2] - center) / scale * bbox_scale + bbox_center
                    )
                else:
    ...
    pose_sequences_2d = collate_pose_sequence(
        pose_results_2d_copy, with_track_id, target_idx
    )
    ...
    for i, pose_seq in enumerate(pose_sequences_2d):
        ...
        keypoints_2d = (
            np.squeeze(keypoints_2d, axis=0) if keypoints_2d.ndim == 4 else keypoints_2d
        )
        ...
        data_info["lifting_target_visible"] = np.ones((1, K, 1), dtype=np.float32)
