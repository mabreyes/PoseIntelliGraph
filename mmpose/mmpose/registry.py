...
TRANSFORMS = Registry(
    "transform", parent=MMENGINE_TRANSFORMS, locations=["mmpose.datasets.transforms"]
)
...
MODEL_WRAPPERS = Registry(
    "model_wrapper", parent=MMENGINE_MODEL_WRAPPERS, locations=["mmpose.models"]
)
...
OPTIM_WRAPPERS = Registry(
    "optimizer_wrapper", parent=MMENGINE_OPTIM_WRAPPERS, locations=["mmpose.engine"]
)
...
EVALUATORS = Registry(
    "evaluator", parent=MMENGINE_EVALUATOR, locations=["mmpose.evaluation.evaluators"]
)

# manage task-specific modules like anchor generators and box coders
TASK_UTILS = Registry(
    "task util", parent=MMENGINE_TASK_UTILS, locations=["mmpose.models.task_modules"]
)

# Registries For Visualizer and the related
# manage visualizer
VISUALIZERS = Registry(
    "visualizer", parent=MMENGINE_VISUALIZERS, locations=["mmpose.visualization"]
)
# manage visualizer backend
VISBACKENDS = Registry(
    "vis_backend", parent=MMENGINE_VISBACKENDS, locations=["mmpose.visualization"]
)

# manage all kinds log processors
LOG_PROCESSORS = Registry(
    "log processor", parent=MMENGINE_LOG_PROCESSORS, locations=["mmpose.visualization"]
)
...
INFERENCERS = Registry(
    "inferencer", parent=MMENGINE_INFERENCERS, locations=["mmpose.apis.inferencers"]
)
