...
class MPJPE(BaseMetric):
    ...
    def __init__(
        ...
        if mode not in allowed_modes:
            raise KeyError(
                "`mode` should be 'mpjpe', 'p-mpjpe', or "
    ...
    def process(self, data_batch: Sequence[dict], data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            mask = (
                gt["lifting_target_visible"]
                .astype(bool)
                .reshape(gt_coords.shape[0], -1)
            )
            # instance action
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        metrics[error_name] = keypoint_mpjpe(
            pred_coords, gt_coords, mask, self.ALIGNMENT[self.mode]
        )
