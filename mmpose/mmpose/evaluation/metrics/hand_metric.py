...
class InterHandMetric(BaseMetric):
    ...
    def process(self, data_batch: Sequence[dict], data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            if "MPJPE" in self.modes:
                ...
                pred_coords_cam = pixel_to_camera(
                    pred_coords_cam,
                    focal[0],
                    focal[1],
                    principal_pt[0],
            ...
            if "MRRPE" in self.modes:
                ...
                if (
                    gt_hand_type.all()
                    and keypoints_visible[..., 20]
                    and keypoints_visible[..., 41]
                ):
                    ...
                    pred_left_root_coords[..., 2] += abs_depth[0] + pred_root_depth
                    ...
                    pred_rel_root_coords = (
                        pred_left_root_coords - pred_right_root_coords
                    )
                    pred_rel_root_coords = np.expand_dims(pred_rel_root_coords, axis=0)
                    gt_rel_root_coords = gt_coords[..., 41, :] - gt_coords[..., 20, :]
                    gt_rel_root_coords = np.expand_dims(gt_rel_root_coords, axis=0)
                else:
                    rel_root_mask = np.array([False])
                    pred_rel_root_coords = np.array([[0, 0, 0]])
                    pred_rel_root_coords = pred_rel_root_coords.reshape(1, 1, 3)
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if "MPJPE" in self.modes:
            ...
            single_mask = np.concatenate([result["single_mask"] for result in results])
            ...
            metrics["MPJPE_single"] = keypoint_epe(pred_coords, gt_coords, single_mask)
        ...
        if "MRRPE" in self.modes:
            ...
            rel_root_mask = np.array([result["rel_root_mask"] for result in results])
            metrics["MRRPE"] = keypoint_epe(
                pred_rel_root_coords, gt_rel_root_coords, rel_root_mask
            )
        return metrics
