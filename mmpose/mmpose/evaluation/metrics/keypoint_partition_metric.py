...
class KeypointPartitionMetric(BaseMetric):
    ...
    def __init__(
        ...
        super().__init__()
        # check metric type
        supported_metric_types = ["CocoMetric", "PCKAccuracy", "AUC", "EPE", "NME"]
        ...
        for partition_name in partitions.keys():
            ...
            if "outfile_prefix" in _metric:
                _metric["outfile_prefix"] = (
                    _metric["outfile_prefix"] + "." + partition_name
                )
        ...
        for data_sample in data_samples:
            for partition_name, keypoint_ids in self.partitions.items():
                ...
                if "raw_ann_info" in _data_sample:
                    ...
                    anns = (
                        raw_ann_info
                        if isinstance(raw_ann_info, list)
                        else [raw_ann_info]
                    )
                    for ann in anns:
                        if "keypoints" in ann:
