...
class PoseTrack18Metric(CocoMetric):
    ...
    def results2json(self, keypoints: Dict[int, list], outfile_prefix: str) -> str:
        ...
        gt_folder = osp.join(
            osp.dirname(self.ann_file), osp.splitext(self.ann_file.split("_")[-1])[0]
        )
        ...
        for json_file in json_files:
            ...
            for image in gt["images"]:
                ...
                for track_id, img_kpt in enumerate(img_kpts):
                    ...
                    ann["keypoints"] = (
                        np.array(img_kpt["keypoints"]).reshape(-1).tolist()
                    )
                    ann["scores"] = (
                        np.array(ann["keypoints"]).reshape([-1, 3])[:, 2].tolist()
                    )
