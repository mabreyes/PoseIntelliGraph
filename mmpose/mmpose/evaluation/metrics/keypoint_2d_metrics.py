...
class PCKAccuracy(BaseMetric):
    ...
    def __init__(
        ...
        self.thr = thr
        self.norm_item = (
            norm_item if isinstance(norm_item, (tuple, list)) else [norm_item]
        )
    ...
    def process(self, data_batch: Sequence[dict], data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            if "bbox" in self.norm_item:
                assert "bboxes" in gt, (
                    "The ground truth data info do not "
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if "bbox" in self.norm_item:
            ...
            _, pck, _ = keypoint_pck_accuracy(
                pred_coords, gt_coords, mask, self.thr, norm_size_bbox
            )
            metrics["PCK"] = pck
        ...
        if "head" in self.norm_item:
            ...
            _, pckh, _ = keypoint_pck_accuracy(
                pred_coords, gt_coords, mask, self.thr, norm_size_head
            )
            metrics["PCKh"] = pckh
        ...
        if "torso" in self.norm_item:
            ...
            _, tpck, _ = keypoint_pck_accuracy(
                pred_coords, gt_coords, mask, self.thr, norm_size_torso
            )
            metrics["tPCK"] = tpck
...
class MpiiPCKAccuracy(PCKAccuracy):
    ...
    def __init__(
        ...
        super().__init__(
            thr=thr, norm_item=norm_item, collect_device=collect_device, prefix=prefix
        )
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if "head" in self.norm_item:
            ...
            pck_p, _, _ = keypoint_pck_accuracy(
                pred_coords, gt_coords, mask, self.thr, norm_size_head
            )
            ...
            for r, threshold in enumerate(rng):
                _pck, _, _ = keypoint_pck_accuracy(
                    pred_coords, gt_coords, mask, threshold, norm_size_head
                )
                pckAll[r, :] = 100.0 * _pck
...
class JhmdbPCKAccuracy(PCKAccuracy):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if "bbox" in self.norm_item:
            ...
            pck_p, pck, _ = keypoint_pck_accuracy(
                pred_coords, gt_coords, mask, self.thr, norm_size_bbox
            )
            stats = {
                "Head PCK": pck_p[2],
...
class AUC(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        auc = keypoint_auc(
            pred_coords, gt_coords, mask, self.norm_factor, self.num_thrs
        )
...
class NME(BaseMetric):
    ...
    def __init__(
        ...
        if norm_mode not in allowed_norm_modes:
            raise KeyError(
                "`norm_mode` should be 'use_norm_item' or "
    ...
    def _get_normalize_factor(self, gt_coords: np.ndarray) -> np.ndarray:
        ...
        interocular = np.linalg.norm(
            gt_coords[:, idx1, :] - gt_coords[:, idx2, :], axis=1, keepdims=True
        )
