...
class BaseMocapDataset(BaseDataset):
    ...
    def __init__(
        ...
        assert 0 < subset_frac <= 1, (
            f"Unsupported `subset_frac` {subset_frac}. Supported range " "is (0, 1]."
        )
    ...
    def get_img_info(self, img_idx, img_name):
        try:
            with get_local_path(
                osp.join(self.data_prefix["img"], img_name)
            ) as local_path:
    ...
    def _get_bottomup_data_infos(
        self, instance_list: List[Dict], image_list: List[Dict]
    ) -> List[Dict]:
