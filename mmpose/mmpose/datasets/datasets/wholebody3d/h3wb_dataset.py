        ...
        for subject in self.subjects:
            ...
            for act in actions:
                for cam in self.camera_order_id:
                    ...
                    camera_param["f"] = (
                        camera_param["K"][0, 0] * 1000,
                        camera_param["K"][1, 1] * 1000,
                    )
                    camera_param["c"] = (
                        camera_param["K"][0, 2] * 1000,
                    ...
                    seq_indices = [
                        ...
                        for i in list(range(0, len(_indices) - _len + 1))
                    ]
                    ...
                    for idx, frame_ids in enumerate(seq_indices):
                        ...
                        bbox = self.bboxes[(subject, act, cam, frames[frame_ids[-1]])]
                        bbox = np.array(
                            [
                                [
                                    bbox["x_min"],
                        ...
                        instance_info = {
                            ...
                            "keypoints_visible": np.ones_like(
                                _kpts_2d[..., 0], dtype=np.float32
                            ),
                            "keypoints_3d_visible": np.ones_like(
                                _kpts_2d[..., 0], dtype=np.float32
                            ),
                            "bbox": bbox,
                            "bbox_score": np.ones((len(frame_ids),)),
                            ...
                            "lifting_target_visible": np.ones_like(
                                _kpts_2d[..., 0], dtype=np.float32
                            )[target_idx],
                        }
