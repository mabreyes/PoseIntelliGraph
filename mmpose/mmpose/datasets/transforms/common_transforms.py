...
class PhotometricDistortion(BaseTransform):
    ...
    def _convert(
        self, img: np.ndarray, alpha: float = 1, beta: float = 0
    ) -> np.ndarray:
    ...
    def transform(self, results: dict) -> dict:
        ...
        if hsv_mode:
            ...
            if hsv_mode == 1 or hsv_mode == 3:
                ...
                img[:, :, 1] = self._convert(img[:, :, 1], alpha=saturation_alpha)
...
class GenerateTarget(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if not isinstance(self.encoder, list):
            ...
            for _encoder in self.encoder:
                auxiliary_encode_kwargs = {
                    key: results[key] for key in _encoder.auxiliary_encode_keys
                }
                encoded_list.append(
                    _encoder.encode(
                        keypoints=keypoints,
    ...
    def __repr__(self) -> str:
        ...
        repr_str += (
            "use_dataset_keypoint_weights=" f"{self.use_dataset_keypoint_weights})"
        )
        return repr_str
