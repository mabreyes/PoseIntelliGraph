...
class BottomupGetHeatmapMask(BaseTransform):
    ...
    def _segs_to_mask(self, segs: list, img_shape: Tuple[int, int]) -> np.ndarray:
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if "warp_mat" in results:
            ...
            mask = cv2.warpAffine(mask, warp_mat, input_size, flags=cv2.INTER_LINEAR)
        ...
        if "heatmaps" in results:
            ...
            if isinstance(heatmaps, list):
                ...
                for hm in results["heatmaps"]:
                    h, w = hm.shape[1:3]
                    _mask = imresize(mask, size=(w, h), interpolation="bilinear")
                    heatmap_mask.append(_mask)
            else:
                h, w = heatmaps.shape[1:3]
                heatmap_mask = imresize(mask, size=(w, h), interpolation="bilinear")
        else:
            heatmap_mask = mask
...
class BottomupRandomAffine(BaseTransform):
    ...
    def _random(
        self, low: float = -1.0, high: float = 1.0, size: tuple = ()
    ) -> np.ndarray:
        if self.distribution == "trunc_norm":
            ...
            raise ValueError(
                f"the argument `distribution` should be either"
    ...
    def _get_transform_params(self) -> Tuple:
        ...
        if np.random.rand() < self.scale_prob:
            ...
            scale = (
                scale_min + (scale_max - scale_min) * (self._random(size=(1,)) + 1) / 2
            )
        else:
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        if "affine" in self.transform_mode:
            ...
            if self.transform_mode == "affine_udp":
                center = np.array(
                    [(img_w - 1.0) / 2, (img_h - 1.0) / 2], dtype=np.float32
                )
                warp_mat = get_udp_warp_matrix(
                    center=center + offset, scale=scale, rot=rotate, output_size=(w, h)
                )
            else:
        ...
        if "keypoints" in results:
            ...
            if self.clip_border:
                (
                    ...
                    results["keypoints"], results["keypoints_visible"], (w, h)
                )
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        for i, (_w, _h) in enumerate(input_sizes):
            ...
            if self.use_udp:
                ...
                warp_mat = get_udp_warp_matrix(
                    center=center, scale=scale, rot=0, output_size=actual_input_size
                )
            else:
                center = np.array([img_w / 2, img_h / 2], dtype=np.float32)
                scale = np.array(
                ...
                warp_mat = get_warp_matrix(
                    center=center, scale=scale, rot=0, output_size=padded_input_size
                )
    ...
    def _crop_data(
        ...
        if results.get("bbox", None) is not None:
                ...
                if self.bbox_clip_border:
                    ...
                    kpt_outside = np.logical_or.reduce(
                        (
                            keypoints_outside_x,
                            keypoints_outside_y,
                            keypoints_outside_width,
                            keypoints_outside_height,
                        )
                    )
            ...
            if results.get("masks", None) is not None:
                results["masks"] = results["masks"][valid_inds.nonzero()[0]].crop(
...
class BottomupRandomChoiceResize(BaseTransform):
    ...
    def _resize_bboxes(self, results: dict) -> None:
        ...
        if results.get("bbox", None) is not None:
            bboxes = results["bbox"] * np.tile(np.array(results["scale_factor"]), 2)
