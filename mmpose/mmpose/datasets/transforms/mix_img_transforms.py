...
class Mosaic(MixImageTransform):
    ...
    def apply_mix(self, results: dict) -> dict:
        ...
        bboxes = bbox_clip_border(
            bboxes, (2 * self.img_scale[0], 2 * self.img_scale[1])
        )
        kpts, kpts_vis = keypoint_clip_border(
            kpts, kpts_vis, (2 * self.img_scale[0], 2 * self.img_scale[1])
        )
    ...
    def _mosaic_combine(
        ...
        if loc == "top_left":
            ...
            x1, y1, x2, y2 = (
                cx,
                cy,
                min(cx + w, self.img_scale[0] * 2),
                min(self.img_scale[1] * 2, cy + h),
            )
...
class YOLOXMixUp(MixImageTransform):
    ...
    def _create_mixup_image(self, results, mixed_data_list):
        ...
        # init mixup image
        out_img = (
            np.ones((self.img_scale[1], self.img_scale[0], 3), dtype=aux_img.dtype)
            * self.pad_val
        )
        annos = defaultdict(list)
        ...
        scale_ratio = min(
            self.img_scale[1] / aux_img.shape[0], self.img_scale[0] / aux_img.shape[1]
        )
        aux_img = mmcv.imresize(
            aux_img,
            (int(aux_img.shape[1] * scale_ratio), int(aux_img.shape[0] * scale_ratio)),
        )
        ...
        kpts, kpts_vis = keypoint_clip_border(
            kpts, aux_results["keypoints_visible"], (aux_w, aux_h)
        )
        if is_filp:
            kpts, kpts_vis = flip_keypoints(
                kpts, kpts_vis, (aux_w, aux_h), aux_results["flip_indices"]
            )
        kpts[..., 0] -= dx
        kpts[..., 1] -= dy
