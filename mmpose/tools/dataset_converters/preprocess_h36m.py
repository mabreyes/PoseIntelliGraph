...
class PreprocessH36m:
    ...
    def __init__(
        ...
        if extracted_dir is None:
            self.extracted_dir = join(
                os.path.dirname(os.path.abspath(self.original_dir)), "extracted"
            )
        else:
        ...
        if processed_dir is None:
            self.processed_dir = join(
                os.path.dirname(os.path.abspath(self.original_dir)), "processed"
            )
        else:
        ...
        self.image_sizes = {
            "54138969": {"width": 1000, "height": 1002},
            "55011271": {"width": 1000, "height": 1000},
            "58860488": {"width": 1000, "height": 1000},
            "60457274": {"width": 1000, "height": 1002},
        }
    ...
    def generate_annotations(self):
        ...
        output_dir = join(
            self.processed_dir, "annotation_body3d", f"fps{50 // self.sample_rate}"
        )
                ...
                for action, subaction in self.sequence_mappings[subject].keys():
                    ...
                    for camera in self.camera_ids:
                        (
                            ...
                            kps2d,
                            kps3d,
                        ) = self._load_annotations(subject, action, subaction, camera)
    ...
    def _get_base_filename(self, subject, action, subaction, camera):
        ...
        return f"{self.sequence_mappings[subject][(action, subaction)]}" + f".{camera}"
    ...
    def _get_camera_params(self, camera, subject):
        ...
        metadata_slice[6:] = self.cameras_raw[
            265 + camera * 9 - 1 : 265 + (camera + 1) * 9 - 1
        ]
    ...
    def _load_annotations(self, subject, action, subaction, camera):
        ...
        # load 2D keypoints
        with pycdf.CDF(
            join(subj_dir, "MyPoseFeatures", "D2_Positions", basename + ".cdf")
        ) as cdf:
            kps_2d = np.array(cdf["Pose"])

        num_frames = kps_2d.shape[1]
        kps_2d = kps_2d.reshape((num_frames, 32, 2))[
            :: self.sample_rate, self.movable_joints
        ]
        kps_2d = np.concatenate([kps_2d, np.ones((len(kps_2d), 17, 1))], axis=2)

        # load 3D keypoints
        with pycdf.CDF(
            join(subj_dir, "MyPoseFeatures", "D3_Positions_mono", basename + ".cdf")
        ) as cdf:
            kps_3d = np.array(cdf["Pose"])

        kps_3d = (
            kps_3d.reshape((num_frames, 32, 3))[
                :: self.sample_rate, self.movable_joints
            ]
            / 1000.0
        )
        kps_3d = np.concatenate([kps_3d, np.ones((len(kps_3d), 17, 1))], axis=2)
        ...
        scales = self.scale_factor * np.max(bboxes[:, 2:] - bboxes[:, :2], axis=1) / 200
