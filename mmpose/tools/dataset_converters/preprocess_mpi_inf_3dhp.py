        ...
        for seq in train_seqs:
            ...
            with open(camera_file, "r") as fin:
                ...
                for cam in train_cams:
                    ...
                    f = np.array([[K[0]], [K[5]]])
                    c = np.array([[K[2]], [K[6]]])
                    RT = np.array([float(s) for s in lines[cam * 7 + 6][11:-2].split()])
            ...
            for cam in train_cams:
                ...
                joints_2d = np.reshape(
                    annot2[cam][0][:num_frames], (num_frames, 28, 2)
                )[:, joints_17]
                joints_3d = np.reshape(
                    annot3[cam][0][:num_frames], (num_frames, 28, 3)
                )[:, joints_17]
                ...
                video_path = join(seq_path, "imageSequence", f"video_{cam}.avi")
...
def load_testset(data_root, out_dir, valid_only=True):
    ...
    for subj in test_subjects:
        ...
        centers, scales, joints_2d, joints_3d = get_annotations(joints_2d, joints_3d)
        _centers.append(centers)
        _scales.append(scales)
        _joints_2d.append(joints_2d)
        _joints_3d.append(joints_3d)
    ...
    print(
        f"Create annotation file for testset: {out_file}. "
        f"{len(_imgnames)} samples in total."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("out_dir", type=str, help="directory to save annotation files.")
