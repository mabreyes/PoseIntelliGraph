...
def xml2coco_trainval(file_list, img_root, save_path, start_ann_id=0):
    ...
    for file in file_list:
        ...
        img_id = int(
            data_anno["image"].split("_")[0] + data_anno["image"].split("_")[1]
        )
    ...
    cocotype["info"] = {}
    cocotype["info"]["description"] = "AnimalPose dataset Generated by MMPose Team"
...
def xml2coco_test(file_list, img_root, save_path, start_ann_id=0):
    ...
    for file in file_list:
        ...
        img_id = category_id * 1000 + int(re.findall(r"\d+", data_anno["image"])[0])

        assert img_id not in img_ids

        # prepare images
        image_name = os.path.join(
            "animalpose_image_part2", data_anno["category"], data_anno["image"]
        )
        img = cv2.imread(os.path.join(img_root, image_name))
        ...
        anno["bbox"] = [
            float(visible_bounds["@xmin"]),
            float(visible_bounds["@xmax"]),  # typo in original xml: should be 'ymin'
            float(visible_bounds["@width"]),
            float(visible_bounds["@height"]),
        ]
        anno["iscrowd"] = 0
...
def split_train_val(work_dir, trainval_file, train_file, val_file, val_ann_num):
    ...
    if count == val_ann_num:
        ...
        warnings.warn(f"We only found {count} annotations, instead of {val_ann_num}.")
    ...
    cocotype_val["info"] = {}
    cocotype_val["info"]["description"] = "AnimalPose dataset Generated by MMPose Team"
    cocotype_val["info"]["version"] = "1.0"
    cocotype_val["info"]["year"] = time.strftime("%Y", time.localtime())
    cocotype_val["info"]["date_created"] = time.strftime("%Y/%m/%d", time.localtime())
    cocotype_val["images"] = images_val
    cocotype_val["annotations"] = annotations_val
    cocotype_val["categories"] = category_info

    json.dump(cocotype_val, open(os.path.join(work_dir, val_file), "w"), indent=4)
    print("=========================================================")
