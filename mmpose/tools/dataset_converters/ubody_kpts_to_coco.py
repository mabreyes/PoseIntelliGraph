    ...
    for scene in folders:
        ...
        for aid in data.anns.keys():
            ...
            img_path = os.path.join(
                annotation_path.replace("annotations", "images"), scene, file_name
            )
        ...
        scene_train_data = dict(
            images=scene_train_imgs, annotations=scene_train_anns, categories=categories
        )
        scene_val_data = dict(
            images=scene_val_imgs, annotations=scene_val_anns, categories=categories
        )
        ...
        mmengine.dump(
            scene_val_data, os.path.join(annotation_path, scene, "val_annotations.json")
        )

    train_data = dict(images=train_imgs, annotations=train_annos, categories=categories)
    val_data = dict(images=val_imgs, annotations=val_annos, categories=categories)

    mmengine.dump(train_data, os.path.join(annotation_path, "train_annotations.json"))
    mmengine.dump(val_data, os.path.join(annotation_path, "val_annotations.json"))
