...
def process_scene_anno(
    scene: str, annotation_root: str, splits: np.array, human_model_path: str
):
    ...
    for aid in keypoint_annos.anns.keys():
        ...
        if "lhand_pose" in human_model_param and human_model_param.get(
            ...
            lhand_pose = np.zeros(
                (3 * len(human_model.orig_joint_part["lhand"])), dtype=np.float32
            )
        ...
        if "rhand_pose" in human_model_param and human_model_param.get(
            ...
            rhand_pose = np.zeros(
                (3 * len(human_model.orig_joint_part["rhand"])), dtype=np.float32
            )
        ...
        joint_cam = (
            joint_cam - joint_cam[human_model.root_joint_idx, None, :]
        )  # root-relative
        ...
        joint_cam[human_model.joint_part["face"], :] = (
            joint_cam[human_model.joint_part["face"], :]
            - joint_cam[human_model.neck_idx, None, :]
        )  # face root-relative
        ...
        joint_img[human_model.joint_part["body"], 2] = (
            (
                joint_cam[human_model.joint_part["body"], 2].copy() / (body_3d_size / 2)
                + 1
            )
            / 2.0
            * output_hm_shape[0]
        ...
        joint_img[human_model.joint_part["lhand"], 2] = (
            (
                joint_cam[human_model.joint_part["lhand"], 2].copy()
                / (body_3d_size / 2)
                + 1
            )
            / 2.0
        ...
        joint_img[human_model.joint_part["rhand"], 2] = (
            (
                joint_cam[human_model.joint_part["rhand"], 2].copy()
                / (body_3d_size / 2)
                + 1
            )
            / 2.0
        ...
        joint_img[human_model.joint_part["face"], 2] = (
            (
                joint_cam[human_model.joint_part["face"], 2].copy() / (body_3d_size / 2)
                + 1
            )
            / 2.0
            * output_hm_shape[0]
    ...
    mmengine.dump(
        train_data, osp.join(annotation_root, scene, "train_3dkeypoint_annotation.json")
    )
    mmengine.dump(
        val_data, osp.join(annotation_root, scene, "val_3dkeypoint_annotation.json")
    )
