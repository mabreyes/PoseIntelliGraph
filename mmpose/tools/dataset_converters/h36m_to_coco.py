...
def _keypoint_camera_to_world(
    keypoints, camera_params, image_name=None, dataset="Body3DH36MDataset"
):
...
def main():
    ...
    h36m_imgs = mmengine.track_parallel_progress(_get_img_info, tasks, nproc=12)
    ...
    tasks = [
        (idx,) + args + (h36m_camera_params,)
        for idx, args in enumerate(zip(kpts_2d, kpts_3d, centers, scales, imgnames))
    ]
