...
def generate_dup_file_name(out_file):
    ...
    if out_file and osp.exists(out_file):
        ...
        exist_files = tuple(
            filter(lambda f: f.startswith(img_name), os.listdir(osp.dirname(out_file)))
        )
...
def main():
    ...
    if args.mode == "original":
        ...
        cfg[f"{args.phase}_dataloader"].dataset.pipeline[-1].pack_transformed = True
    ...
    if isinstance(dataset, CombinedDataset):
        ...
        def generate_index_generator(
            dataset_starting_indexes: list, max_item_datasets: int
        ):
        ...
        max_item_datasets = [
            min(dataset_len, args.max_item_per_dataset) for dataset_len in dataset.lens
        ]

        # Generate indexes using the generator
        indexes = generate_index_generator(dataset_starting_indexes, max_item_datasets)
    ...
    for idx in indexes:
        ...
        if args.mode == "original":
            ...
            if item["keypoints_visible"].ndim == 3:
                gt_instances.keypoints_visible = item["keypoints_visible"][..., 0]
            else:
        ...
        output_dir = (
            osp.join(args.output_dir, dataset_name)
            if multiple_datasets and dataset_name
            else args.output_dir
        )
        out_file = (
            osp.join(output_dir, osp.basename(img_path))
