...
class TestPose2DInferencer(TestCase):
    ...
    def test_init(self):
        ...
        inferencer = Pose2DInferencer(
            ...
            det_model=det_model,
            det_weights=det_weights,
            det_cat_ids=(15, 16, 17, 18, 19, 20, 21, 22, 23) if det_model else None,
        )
        self.assertIsInstance(inferencer.model, torch.nn.Module)
        self.assertIsInstance(inferencer.detector, BaseInferencer)
        self.assertSequenceEqual(
            inferencer.det_cat_ids, (15, 16, 17, 18, 19, 20, 21, 22, 23)
        )
    ...
    def test_call(self):
        ...
        inputs = (
            "tests/data/posetrack18/videos/000001_mpiinew_test/"
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn(
                "000001_mpiinew_test.mp4", os.listdir(f"{tmp_dir}/visualizations")
            )
            self.assertIn(
                "000001_mpiinew_test.json", os.listdir(f"{tmp_dir}/predictions")
            )
