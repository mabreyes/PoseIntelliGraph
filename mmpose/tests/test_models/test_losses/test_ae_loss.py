...
class TestAssociativeEmbeddingLoss(TestCase):
    def _make_input(self, num_instance: int) -> Tuple[Tensor, Tensor]:
        encoder = AssociativeEmbedding(input_size=(256, 256), heatmap_size=(64, 64))

        data = get_coco_sample(img_shape=(256, 256), num_instances=num_instance)
    ...
    def test_loss(self):
        ...
        # test loss weight
        loss_module = AssociativeEmbeddingLoss(loss_weight=0.0)
        pull_loss, push_loss = loss_module(tags, keypoint_indices)
        ...
        loss_module = AssociativeEmbeddingLoss(push_loss_factor=0.0)
        pull_loss, push_loss = loss_module(tags, keypoint_indices)
