...
class TestAdaptiveWingLoss(TestCase):
    def test_loss(self):
        ...
        self.assertTrue(torch.allclose(loss(fake_pred, fake_label), torch.tensor(0.0)))

        fake_pred = torch.ones((1, 3, 2, 2))
        fake_label = torch.zeros((1, 3, 2, 2))
        self.assertTrue(
            torch.allclose(loss(fake_pred, fake_label), torch.tensor(8.4959), atol=1e-4)
        )
        ...
        self.assertTrue(
            torch.allclose(loss(fake_pred, fake_label, fake_weight), torch.tensor(0.0))
        )
...
class TestFocalHeatmapLoss(TestCase):
    def test_loss(self):
        ...
        self.assertTrue(
            torch.allclose(loss(fake_pred, fake_label), torch.tensor(0.1569), atol=1e-4)
        )
...
class TestKeypointMSELoss(TestCase):
    def test_loss(self):
        ...
        loss = KeypointMSELoss(use_target_weight=False, skip_empty_channel=False)
        ...
        self.assertTrue(
            torch.allclose(loss(fake_pred, fake_label), torch.tensor(0.0), atol=1e-4)
        )

        # test loss w/ target_weight and without mask
        loss = KeypointMSELoss(use_target_weight=True, skip_empty_channel=False)

        fake_weight = torch.ones((1, 4)).float()
        self.assertTrue(
            torch.allclose(
                loss(fake_pred, fake_label, fake_weight), torch.tensor(0.0), atol=1e-4
            )
        )
