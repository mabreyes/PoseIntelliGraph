...
class TestDSNTHead(TestCase):
    ...
    def test_init(self):
        # square heatmap
        head = DSNTHead(in_channels=32, in_featuremap_size=(8, 8), num_joints=17)
        ...
        # rectangle heatmap
        head = DSNTHead(in_channels=32, in_featuremap_size=(6, 8), num_joints=17)
    ...
    def test_predict(self):
        ...
        batch_data_samples = get_packed_inputs(batch_size=2, with_reg_label=False)[
            "data_samples"
        ]
    ...
    def test_tta(self):
        ...
        preds = head.predict(
            [feats, feats], batch_data_samples, test_cfg=dict(flip_test=True)
        )
    ...
    def test_loss(self):
        for dist_loss in ["l1", "l2"]:
            ...
            feats = self._get_feats(batch_size=2, feat_shapes=[(32, 8, 6)])
            batch_data_samples = get_packed_inputs(batch_size=2, with_reg_label=True)[
                "data_samples"
            ]
