...
class TestCPMHead(TestCase):
    ...
    def test_init(self):
        # w/o deconv
        head = CPMHead(
            num_stages=1, in_channels=256, out_channels=17, deconv_out_channels=None
        )
        ...
        # w/o final layer
        head = CPMHead(num_stages=6, in_channels=17, out_channels=17, final_layer=None)
    ...
    def test_predict(self):
        decoder_cfg = dict(
            type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(24, 32), sigma=2.0
        )
    ...
    def test_loss(self):
        ...
        head = CPMHead(
            ...
            out_channels=17,
            final_layer=None,
            loss=[dict(type="KeypointMSELoss", use_target_weight=True)] * num_stages,
        )
        ...
        feats = self._get_feats(batch_size=2, feat_shapes=[(17, 32, 24)] * num_stages)
        batch_data_samples = self._get_data_samples(batch_size=2)
        losses = head.loss(feats, batch_data_samples)
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "did not match `num_stages`"):
            _ = CPMHead(
                ...
                in_channels=17,
                out_channels=17,
                loss=[dict(type="KeypointMSELoss", use_target_weight=True)] * 4,
            )
