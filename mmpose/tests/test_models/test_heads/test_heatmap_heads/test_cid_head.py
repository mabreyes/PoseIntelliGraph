...
class TestCIDHead(TestCase):
    ...
    def _get_data_samples(self):
        data_samples = get_packed_inputs(
            1, input_size=(512, 512), heatmap_size=(128, 128), img_shape=(512, 512)
        )["data_samples"]
        return data_samples
    ...
    def test_predict(self):
        ...
        head = CIDHead(in_channels=32, num_keypoints=17, gfd_channels=32, decoder=codec)
        ...
        # output heatmaps
        with torch.no_grad():
            _, pred_fields = head.predict(
                feats, data_samples, dict(output_heatmaps=True)
            )
    ...
    def test_loss(self):
        ...
        encoded = head.decoder.encode(
            data["keypoints"], data["keypoints_visible"], data["bbox"]
        )
        feats = self._get_feats()
        data_samples = self._get_data_samples()
        for data_sample in data_samples:
            data_sample.gt_fields.set_data(
                {
                    "heatmaps": to_tensor(encoded["heatmaps"]),
                    "instance_heatmaps": to_tensor(encoded["instance_heatmaps"]),
                }
            )
