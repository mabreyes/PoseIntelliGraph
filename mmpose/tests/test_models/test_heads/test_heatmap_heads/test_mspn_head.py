...
class TestMSPNHead(TestCase):
    def _get_feats(
        ...
        for i in range(num_stages):
            ...
            for j in range(num_units):
                feats_units.append(
                    torch.rand((batch_size,) + feat_shapes[j], dtype=torch.float32)
                )
            feats_stages.append(feats_units)

        return feats_stages

    def _get_data_samples(
        self, batch_size: int = 2, heatmap_size=(48, 64), num_levels=1
    ):
    ...
    def test_loss(self):
        ...
        with self.assertRaisesRegex(
            AssertionError, "length of feature maps did not match the `num_stages`"
        ):
        ...
        with self.assertRaisesRegex(
            AssertionError, "length of feature maps did not match the `num_units`"
        ):
            feats = self._get_feats(
                num_stages=1,
                num_units=2,
        ...
        with self.assertRaisesRegex(
            AssertionError, "number of feature map channels did not match"
        ):
            feats = self._get_feats(
                num_stages=1,
                num_units=4,
        ...
        for ds in batch_data_samples:
            ds.gt_instance_labels = InstanceData(
                keypoint_weights=ds.gt_instance_labels.keypoint_weights.transpose(0, 1)
            )
        ...
        head = MSPNHead(
            ...
            out_channels=17,
            loss=(
                [dict(type="KeypointMSELoss", use_target_weight=True, loss_weight=0.25)]
                * 3
                + [
                    dict(
    ...
    def test_predict(self):
        ...
        self.assertEqual(
            preds[0].keypoints.shape, batch_data_samples[0].gt_instances.keypoints.shape
        )
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "The length of loss_module"):
            _ = MSPNHead(
                ...
                out_channels=17,
                level_indices=[0, 1, 2, 3, 1, 2, 3, 4],
                loss=[dict(type="KeypointMSELoss", use_target_weight=True)] * 3,
            )
