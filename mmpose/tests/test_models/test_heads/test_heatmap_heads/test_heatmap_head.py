...
class TestHeatmapHead(TestCase):
    ...
    def test_init(self):
        # w/o deconv
        head = HeatmapHead(in_channels=32, out_channels=17, deconv_out_channels=None)
    ...
    def test_tta(self):
        ...
        head = HeatmapHead(in_channels=32, out_channels=17 * 3, decoder=decoder_cfg)
        feats = self._get_feats(batch_size=2, feat_shapes=[(32, 8, 6)])
        batch_data_samples = get_packed_inputs(batch_size=2)["data_samples"]
        preds = head.predict(
            [feats, feats],
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "Got mismatched lengths"):
            _ = HeatmapHead(in_channels=32, out_channels=17, conv_out_channels=(256,))
