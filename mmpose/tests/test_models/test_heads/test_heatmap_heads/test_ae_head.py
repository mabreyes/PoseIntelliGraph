...
class TestAssociativeEmbeddingHead(TestCase):
    ...
    def test_predict(self):
        ...
        batch_data_samples = get_packed_inputs(
            1, input_size=(256, 256), heatmap_size=(64, 64), img_shape=(256, 256)
        )["data_samples"]

        keypoints = batch_data_samples[0].gt_instances["keypoints"]
        keypoints_visible = batch_data_samples[0].gt_instances["keypoints_visible"]
    ...
    def test_loss(self):
        ...
        batch_data_samples[0].gt_fields = PixelData(
            heatmaps=heatmaps, heatmap_mask=heatmap_mask
        ).to_tensor()
        batch_data_samples[0].gt_instance_labels = InstanceData(
            keypoint_indices=keypoint_indices, keypoint_weights=keypoint_weights
        ).to_tensor()
