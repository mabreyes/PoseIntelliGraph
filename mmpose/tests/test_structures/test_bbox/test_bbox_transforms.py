...
class TestGetPersWarpMatrix(TestCase):
    def test_get_pers_warp_matrix_identity(self):
        ...
        warp_matrix = get_pers_warp_matrix(center, translate, scale, rot, shear)

        expected_matrix = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=np.float32)
    ...
    def test_get_pers_warp_matrix_scale_rotation_shear(self):
        ...
        expected_matrix = np.array(
            [
                [1.3448632, -0.77645713, 0.0],
                [1.6730325, 0.44828773, 0.0],
                [0.0, 0.0, 1.0],
            ],
            dtype=np.float32,
        )
        ...
        self.assertTrue(np.allclose(warp_matrix, expected_matrix, rtol=1e-3, atol=1e-3))
...
class TestGetWarpMatrix(TestCase):
    ...
    def test_rotation(self):
        ...
        warp_matrix = get_warp_matrix(center, scale, rot, output_size)
        expected_matrix = np.array(
            [[2.828427, 2.828427, -465.685303], [-2.828427, 2.828427, 100.0]]
        )
        np.testing.assert_array_almost_equal(warp_matrix, expected_matrix)
    ...
    def test_shift(self):
        ...
        warp_matrix = get_warp_matrix(center, scale, rot, output_size, shift=shift)
    ...
    def test_inverse(self):
        ...
        rot = 0
        output_size = (200, 200)
        warp_matrix = get_warp_matrix(center, scale, rot, output_size, inv=True)
