...
class TestRandomFlip(TestCase):
    ...
    def test_transform(self):
        ...
        transform = RandomFlip(prob=1.0, direction="horizontal")
        results = deepcopy(self.data_info)
        results = transform(results)
        ...
        self.assertTrue(np.allclose(results["img"], self.data_info["img"][:, ::-1]))
        self.assertTrue(
            np.allclose(results["img_mask"], self.data_info["img_mask"][:, ::-1])
        )
        self.assertTrue(np.allclose(results["bbox_center"], bbox_center_flipped))
        ...
        # test vertical flip
        transform = RandomFlip(prob=1.0, direction="vertical")
        results = deepcopy(self.data_info)
        results = transform(results)
        ...
        # test diagonal flip
        transform = RandomFlip(prob=1.0, direction="diagonal")
        results = deepcopy(self.data_info)
        results = transform(results)
        ...
        self.assertTrue(np.allclose(results["img"], self.data_info["img"][::-1, ::-1]))
        self.assertTrue(
            np.allclose(results["img_mask"], self.data_info["img_mask"][::-1, ::-1])
        )
        self.assertTrue(np.allclose(results["bbox_center"], bbox_center_flipped))
        self.assertTrue(np.allclose(kpts1[..., 0], 640 - kpts2[..., 0] - 1))
        self.assertTrue(np.allclose(kpts1[..., 1], 480 - kpts2[..., 1] - 1))
    ...
    def test_errors(self):
        # invalid arguments
        with self.assertRaisesRegex(ValueError, "probs must be float or list of float"):
    ...
    def test_repr(self):
        ...
        self.assertEqual(repr(transform), "RandomFlip(prob=0.5, direction=horizontal)")
...
class TestRandomHalfBody(TestCase):
    def setUp(self):
        # prepare dummy top-down data sample with COCO metainfo
        self.data_info = get_coco_sample(
            img_shape=(480, 640), num_instances=1, with_bbox_cs=True, with_img_mask=True
        )
    ...
    def test_transform(self):
        ...
        self.assertTrue(
            np.allclose(
                results["bbox_scale"], (kpts.max(axis=1) - kpts.min(axis=1)) * padding
            )
        )
        ...
        transform = RandomHalfBody(prob=0.0)
...
class TestRandomBBoxTransform(TestCase):
    ...
    def test_transform(self):
        ...
        transform = RandomBBoxTransform(
            shift_factor=shfit_factor, shift_prob=1.0, scale_prob=0.0, rotate_prob=0.0
        )
        results = transform(deepcopy(self.data_info))
        ...
        self.assertTrue(
            (
                (results["bbox_center"] > center_range[0])
                & (results["bbox_center"] < center_range[1])
            ).all()
        )
        self.assertTrue(np.allclose(results["bbox_scale"], scale))
        self.assertTrue(np.allclose(results["bbox_rotation"], np.zeros((1, 17))))

        # test random resizing
        transform = RandomBBoxTransform(
            scale_factor=scale_factor, shift_prob=0.0, scale_prob=1.0, rotate_prob=0.0
        )
        ...
        self.assertTrue(
            (
                (results["bbox_scale"] > scale_range[0])
                & (results["bbox_scale"] < scale_range[1])
            ).all()
        )
        self.assertTrue(np.allclose(results["bbox_rotation"], np.zeros((1, 17))))
        ...
        transform = RandomBBoxTransform(
            rotate_factor=rotate_factor, shift_prob=0.0, scale_prob=0.0, rotate_prob=1.0
        )
        ...
        self.assertTrue(
            (
                (results["bbox_rotation"] > bbox_rotation_min)
                & (results["bbox_rotation"] < bbox_rotation_max)
            ).all()
        )
        ...
        self.assertTrue(
            (
                (results["bbox_rotation"] > rotation_range[0])
                & (results["bbox_rotation"] < rotation_range[1])
            ).all()
        )
...
class TestAlbumentation(TestCase):
    ...
    def test_repr(self):
        ...
        transform = Albumentation(transforms=transforms, keymap={"img": "image"})
        self.assertEqual(repr(transform), f"Albumentation(transforms={transforms})")
...
class TestGenerateTarget(TestCase):
    ...
    def test_generate_single_target(self):
        encoder = dict(
            type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(48, 64), sigma=2.0
        )

        # generate heatmap
        pipeline = Compose(
            [TopdownAffine(input_size=(192, 256)), GenerateTarget(encoder=encoder)]
        )
        results = pipeline(deepcopy(self.data_info))

        self.assertEqual(results["heatmaps"].shape, (17, 64, 48))
        self.assertTrue(np.allclose(results["keypoint_weights"], np.ones((1, 17))))
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(AssertionError, "Need multiple encoder configs"):
        ...
        with self.assertWarnsRegex(DeprecationWarning, "`target_type` is deprecated"):
...
class TestFilterAnnotations(TestCase):
    def setUp(self):
        ...
        self.results = {
            ...
            "img_shape": (224, 224),
            "bbox": np.array([[10, 10, 20, 20], [20, 20, 40, 40], [40, 40, 80, 80]]),
            "bbox_score": np.array([0.9, 0.8, 0.7]),
            "category_id": np.array([1, 2, 3]),
            "keypoints": np.array([[15, 15, 1], [25, 25, 1], [45, 45, 1]]),
            "keypoints_visible": np.array([[1, 1, 0], [1, 1, 1], [1, 1, 1]]),
            "area": np.array([300, 600, 1200]),
        }
    ...
    def test_transform(self):
        ...
        results = transform(copy.deepcopy(self.results))
        print((results["bbox"] == np.array([[20, 20, 40, 40], [40, 40, 80, 80]])).all())
        self.assertTrue(
            (results["bbox"] == np.array([[20, 20, 40, 40], [40, 40, 80, 80]])).all()
        )
        self.assertTrue((results["bbox_score"] == np.array([0.8, 0.7])).all())
        self.assertTrue((results["category_id"] == np.array([2, 3])).all())
        self.assertTrue(
            (results["keypoints"] == np.array([[25, 25, 1], [45, 45, 1]])).all()
        )
        self.assertTrue(
            (results["keypoints_visible"] == np.array([[1, 1, 1], [1, 1, 1]])).all()
        )
        ...
        self.assertTrue((results["bbox"] == np.array([[40, 40, 80, 80]])).all())
        self.assertTrue((results["bbox_score"] == np.array([0.7])).all())
        self.assertTrue((results["category_id"] == np.array([3])).all())
        self.assertTrue((results["keypoints"] == np.array([[45, 45, 1]])).all())
        self.assertTrue((results["keypoints_visible"] == np.array([[1, 1, 1]])).all())
...
class TestYOLOXHSVRandomAug(TestCase):
    def setUp(self):
        ...
        """
        img = mmcv.imread(
            osp.join(osp.dirname(__file__), "../../data/coco/000000000785.jpg"), "color"
        )
        self.results = {
            "img": img,
            "img_shape": (640, 425),
            "category_id": np.array([1, 2, 3], dtype=np.int64),
            "bbox": np.array(
    ...
    def test_transform(self):
        ...
        self.assertTrue(results["category_id"].shape[0] == results["bbox"].shape[0])
