...
class TestPackPoseInputs(TestCase):
    def setUp(self):
        ...
        self.results_topdown = {
            ...
            "keypoint_weights": np.full((1, 17), 1).astype(np.float32),
            "heatmaps": np.random.random((17, 64, 48)).astype(np.float32),
            "keypoint_labels": np.random.randint(0, 100, (1, 17, 2)).astype(np.float32),
            "keypoint_x_labels": np.random.randint(0, 100, (1, 17, 2)).astype(
                np.float32
            ),
            "keypoint_y_labels": np.random.randint(0, 100, (1, 17, 2)).astype(
                np.float32
            ),
            "transformed_keypoints": np.random.randint(0, 100, (1, 17, 2)).astype(
                np.float32
            ),
        }
        self.meta_keys = (
    ...
    def test_transform(self):
        transform = PackPoseInputs(meta_keys=self.meta_keys, pack_transformed=True)
        results = transform(copy.deepcopy(self.results_topdown))
        self.assertIn("transformed_keypoints", results["data_samples"].gt_instances)
        ...
        self.assertIsInstance(results["data_samples"].gt_fields.heatmaps, torch.Tensor)
        self.assertNotIn("transformed_keypoints", results["data_samples"].gt_instances)
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(transform),
            f"PackPoseInputs(meta_keys={self.meta_keys}, "
