...
def get_h36m_sample():
    ...
    ann_info["flip_pairs"] = [[1, 4], [2, 5], [3, 6], [11, 14], [12, 15], [13, 16]]
...
class TestRandomFlipAroundRoot(TestCase):
    def setUp(self):
        ...
        self.keypoints_flip_cfg = dict(center_mode="static", center_x=0.0)
    ...
    def test_transform(self):
        ...
        for left, right in enumerate(flip_indices):
            self.assertTrue(
                np.allclose(-kpts1[0][left][:1], kpts2[0][right][:1], atol=4.0)
            )
            self.assertTrue(
                np.allclose(kpts1[0][left][1:], kpts2[0][right][1:], atol=4.0)
            )
            self.assertTrue(
                np.allclose(tar1[..., left, 1:], tar2[..., right, 1:], atol=4.0)
            )

            self.assertTrue(
                np.allclose(kpts_vis1[..., left], kpts_vis2[..., right], atol=4.0)
            )
            self.assertTrue(
                np.allclose(tar_vis1[..., left], tar_vis2[..., right], atol=4.0)
            )

        # test camera flipping
        transform = RandomFlipAroundRoot(
            self.keypoints_flip_cfg, self.target_flip_cfg, flip_prob=1, flip_camera=True
        )
        results = deepcopy(self.data_info)
        results = transform(results)
        ...
        self.assertTrue(
            np.allclose(
                -self.data_info["camera_param"]["c"][0], camera2["c"][0], atol=4.0
            )
        )
        self.assertTrue(
            np.allclose(
                -self.data_info["camera_param"]["p"][0], camera2["p"][0], atol=4.0
            )
        )
        ...
        transform = RandomFlipAroundRoot(
            self.keypoints_flip_cfg, self.target_flip_cfg, flip_prob=1, flip_label=True
        )
