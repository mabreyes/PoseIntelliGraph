...
class TestBottomupGetHeatmapMask(TestCase):
    ...
    def test_transform(self):
        ...
        results["heatmaps"] = [
            np.random.rand(17, h, w).astype(np.float32) for w, h in heatmap_sizes
        ]
        results = pipeline(results)
...
class TestBottomupResize(TestCase):
    def setUp(self):
        # prepare dummy bottom-up data sample with COCO metainfo
        self.data_info = get_coco_sample(
            img_shape=(240, 480), img_fill=255, num_instances=4, with_bbox_cs=True
        )
    ...
    def test_transform(self):
        ...
        self.assertTrue(np.allclose(results["input_scale"], np.array([480.0, 480.0])))
        self.assertTrue(np.allclose(results["input_center"], np.array([240.0, 120.0])))
...
class TestBottomupRandomCrop(TestCase):
        ...
        for crop_type in crop_type_list:
            ...
            for crop_size in [(0, 0), (0, 1), (1, 0)]:
                with self.assertRaises(AssertionError):
                    BottomupRandomCrop(crop_size=crop_size, crop_type=crop_type)
    ...
    def test_transform(self):
        ...
        for crop_type, crop_size in zip(
            ...
            transform = BottomupRandomCrop(crop_size=crop_size, crop_type=crop_type)
            results = transform(deepcopy(src_results))
            self.assertEqual(results["img"].shape[:2], target_shape)

        # test absolute_range crop
        transform = BottomupRandomCrop(crop_size=(10, 20), crop_type="absolute_range")
        results = transform(deepcopy(src_results))
        ...
        transform = BottomupRandomCrop(crop_size=(0.5, 0.5), crop_type="relative_range")
        results = transform(deepcopy(src_results))
        h, w = results["img"].shape[:2]
        ...
        transform = BottomupRandomCrop(crop_size=(5, 3), allow_negative_crop=False)
        results = transform(deepcopy(src_results))
        ...
        bbox = np.zeros((0, 4), dtype=np.float32)
        src_results = {"img": img, "bbox": bbox}
        transform = BottomupRandomCrop(crop_size=(5, 3), allow_negative_crop=True)
        results = transform(deepcopy(src_results))
...
class TestBottomupRandomChoiceResize(TestCase):
    ...
    def test_transform(self):
        ...
        results = deepcopy(self.data_info)
        results = transform(results)
        self.assertIn((results["img"].shape[1], results["img"].shape[0]), _scale_choice)
        ...
        self.assertTrue((results["bbox"] == np.array([[100, 75, 200, 150]])).all())

        transform = BottomupRandomChoiceResize(
            scales=[(200, 150)], resize_type="Resize", clip_object_border=False
        )
