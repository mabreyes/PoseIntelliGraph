...
class TestPoseTrack18VideoDataset(TestCase):
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(AssertionError, "Invalid `frame_weights`:"):
        ...
        with self.assertRaisesRegex(ValueError, "got invalid frame_sampler_mode"):
            _ = self.build_posetrack18_video_dataset(frame_sampler_mode="invalid")

        # test invalid argument when `frame_sampler_mode = 'random'`
        with self.assertRaisesRegex(AssertionError, "please specify the `frame_range`"):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
                frame_range=None,
            )
        with self.assertRaisesRegex(
            AssertionError, "frame_range can not be a negative value"
        ):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
                frame_range=-2,
        ...
        with self.assertRaisesRegex(AssertionError, "Each element must be int"):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
                frame_range=[-2, 5.5],
            )
        with self.assertRaisesRegex(
            TypeError, "The type of `frame_range` must be int or Sequence"
        ):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
        ...
        with self.assertRaisesRegex(
            AssertionError, "please specify `num_sampled_frame`"
        ):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
        ...
        with self.assertRaisesRegex(
            AssertionError, "does not match the number of sampled adjacent frames"
        ):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
        ...
        with self.assertRaisesRegex(
            AssertionError, "please specify the `frame_indices`"
        ):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="fixed",
                frame_indices=None,
        ...
        with self.assertRaisesRegex(
            ValueError, '"bbox_file" is only supported when `test_mode==True`'
        ):
            _ = self.build_posetrack18_video_dataset(
                test_mode=False,
                bbox_file="tests/data/posetrack18/annotations/"
        ...
        with self.assertRaisesRegex(
            ValueError, '"bbox_score_thr" is only supported in topdown mode'
        ):
            _ = self.build_posetrack18_video_dataset(
                data_mode="bottomup",
                test_mode=True,
