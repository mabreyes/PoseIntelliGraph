...
class TestH36MDataset(TestCase):
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            'Unsupported `keypoint_2d_src` "invalid". '
            f"Supported options are {SUPPORTED_keypoint_2d_src}",
        ):
            _ = self.build_h36m_dataset(
                data_mode="topdown", test_mode=False, keypoint_2d_src="invalid"
            )
        ...
        with self.assertRaisesRegex(AssertionError, "Unsupported `subset_frac` 2."):
