...
class TestKeypointPartitionMetricWrappingCocoMetric(TestCase):
    def setUp(self):
        ...
        self.ann_file_coco = "tests/data/coco/test_keypoint_partition_metric.json"
    ...
    def _assert_outfiles(self, prefix):
        for part in ["body", "foot", "face", "left_hand", "right_hand", "all"]:
            self.assertTrue(
                osp.isfile(
                    osp.join(self.tmp_dir.name, f"{prefix}.{part}.keypoints.json")
                )
            )
    ...
    def test_init(self):
        ...
        with self.assertWarnsRegex(
            UserWarning, "does not support the ann_file argument"
        ):
            _ = KeypointPartitionMetric(
        ...
        with self.assertWarnsRegex(UserWarning, "if score_mode is not 'bbox'"):
            _ = KeypointPartitionMetric(
                metric=dict(type="CocoMetric"), partitions=dict(all=range(133))
            )
...
class TestKeypointPartitionMetricWrappingPCKAccuracy(TestCase):
    ...
    def test_init(self):
        ...
        with self.assertWarnsRegex(
            UserWarning, "norm_item torso is used in JhmdbDataset"
        ):
            _ = KeypointPartitionMetric(
                metric=dict(type="PCKAccuracy", thr=0.05, norm_item=["bbox", "torso"]),
                partitions=dict(all=range(133)),
            )
...
class TestKeypointPartitionMetricWrappingNME(TestCase):
    def setUp(self):
        ...
        for i in range(self.batch_size):
            ...
            gt_instances.keypoints = keypoints
            gt_instances.keypoints_visible = np.ones((1, num_keypoints, 1)).astype(bool)
            gt_instances.keypoints_visible[0, (2 * i) % self.batch_size, 0] = False
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(ValueError, "NME norm_mode 'keypoint_distance'"):
            _ = KeypointPartitionMetric(
    ...
    def test_nme_evaluate(self):
        ...
        nme_metric = KeypointPartitionMetric(
            metric=dict(type="NME", norm_mode="use_norm_item", norm_item="box_size"),
            partitions=dict(
                p1=range(10),
                p2=range(10, 19),
                all=range(19),
