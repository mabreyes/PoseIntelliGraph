...
class TestPCKAccuracy(TestCase):
    def setUp(self):
        ...
        for i in range(self.batch_size):
            ...
            gt_instances.keypoints_visible = np.ones((1, num_keypoints, 1)).astype(bool)
...
class TestAUCandEPE(TestCase):
    def setUp(self):
        ...
        gt_instances.keypoints_visible = np.array([[True, True, False, True, True]])
...
class TestNME(TestCase):
    def _generate_data(
        self, batch_size: int = 1, num_keypoints: int = 5, norm_item: str = "box_size"
    ) -> tuple:
    ...
    def test_nme_evaluate(self):
        ...
        horse10_meta_info = dict(from_file="configs/_base_/datasets/horse10.py")
        horse10_dataset_meta = parse_pose_metainfo(horse10_meta_info)
        nme_metric.dataset_meta = horse10_dataset_meta

        data_batch, data_samples = self._generate_data(batch_size=4, num_keypoints=22)
        nme_metric.process(data_batch, data_samples)
        nme = nme_metric.evaluate(4)
        ...
        data_batch, data_samples = self._generate_data(batch_size=2, num_keypoints=17)
        nme_metric.process(data_batch, data_samples)
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(
            KeyError, "`norm_mode` should be 'use_norm_item' or 'keypoint_distance'"
        ):
        ...
        with self.assertRaisesRegex(
            AssertionError,
            "The ground truth data info do not have the expected " "normalized factor",
        ):
            ...
            data_batch, data_samples = self._generate_data(norm_item="norm_item2")
        ...
        with self.assertRaisesRegex(
            KeyError,
            "can not find the keypoint_indices in " "`DEFAULT_KEYPOINT_INDICES`",
        ):
            nme_metric = NME(norm_mode="keypoint_distance", keypoint_indices=None)
            coco_meta_info = dict(from_file="configs/_base_/datasets/coco.py")
            coco_dataset_meta = parse_pose_metainfo(coco_meta_info)
            nme_metric.dataset_meta = coco_dataset_meta
        ...
        with self.assertRaisesRegex(
            ...
            nme_metric = NME(norm_mode="keypoint_distance", keypoint_indices=[0, 1, 2])
            coco_meta_info = dict(from_file="configs/_base_/datasets/coco.py")
            coco_dataset_meta = parse_pose_metainfo(coco_meta_info)
            nme_metric.dataset_meta = coco_dataset_meta
        ...
        with self.assertRaisesRegex(
            AssertionError, "dataset does not contain the required"
        ):
            nme_metric = NME(norm_mode="keypoint_distance", keypoint_indices=[17, 18])
            coco_meta_info = dict(from_file="configs/_base_/datasets/coco.py")
            coco_dataset_meta = parse_pose_metainfo(coco_meta_info)
            nme_metric.dataset_meta = coco_dataset_meta
