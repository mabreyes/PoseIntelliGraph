...
class TestMPJPE(TestCase):
    def setUp(self):
        ...
        for i in range(self.batch_size):
            ...
            gt_instances.lifting_target = np.random.random((1, num_keypoints, 3))
            gt_instances.lifting_target_visible = np.ones((1, num_keypoints, 1)).astype(
                bool
            )
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(
            KeyError,
            "`mode` should be 'mpjpe', 'p-mpjpe', or 'n-mpjpe', " "but got 'invalid'.",
        ):
            MPJPE(mode="invalid")
