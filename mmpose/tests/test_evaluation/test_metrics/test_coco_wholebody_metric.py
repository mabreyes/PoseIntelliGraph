...
class TestCocoWholeBodyMetric(TestCase):
    def setUp(self):
        ...
        meta_info_coco = dict(from_file="configs/_base_/datasets/coco_wholebody.py")
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(ValueError, "`score_mode` should be one of"):
            _ = CocoWholeBodyMetric(ann_file=self.ann_file_coco, score_mode="invalid")

        # test nms_mode option
        with self.assertRaisesRegex(ValueError, "`nms_mode` should be one of"):
            _ = CocoWholeBodyMetric(ann_file=self.ann_file_coco, nms_mode="invalid")
    ...
    def test_topdown_evaluate(self):
        ...
        metric_coco = CocoWholeBodyMetric(outfile_prefix=f"{self.tmp_dir.name}/test4")
        metric_coco.dataset_meta = self.dataset_meta_coco
        # process samples
        for data_batch, data_samples in self.topdown_data_coco:
            metric_coco.process(data_batch, data_samples)
