...
class TestCocoMetric(TestCase):
    def setUp(self):
        ...
        self.dataset_meta_coco["CLASSES"] = self.coco.loadCats(self.coco.getCatIds())
        ...
        meta_info_crowdpose = dict(from_file="configs/_base_/datasets/crowdpose.py")
        ...
        self.topdown_data_pseudo_coco = self._convert_ann_to_topdown_batch_data(
            self.ann_file_crowdpose, num_pseudo_kpts=3
        )
        assert len(self.topdown_data_crowdpose) == 5
        self.bottomup_data_crowdpose = self._convert_ann_to_bottomup_batch_data(
            self.ann_file_crowdpose
        )
    ...
    def _convert_ann_to_topdown_batch_data(self, ann_file, num_pseudo_kpts: int = 0):
        ...
        for ann in db["annotations"]:
            ...
            if num_pseudo_kpts > 0:
                keypoints = np.concatenate(
                    (keypoints, *((keypoints[:, :1],) * num_pseudo_kpts)), axis=1
                )
            ...
            if "crowdIndex" in imgid2info[ann["image_id"]]:
                data_sample["crowd_index"] = imgid2info[ann["image_id"]]["crowdIndex"]
            # batch size = 1
            data_batch = [data]
    ...
    def _convert_ann_to_bottomup_batch_data(self, ann_file):
        ...
        for img_id, anns in img2ann.items():
            ...
            gt_instances = {"bbox_scores": np.ones((len(anns)), dtype=np.float32)}
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(ValueError, "`score_mode` should be one of"):
        ...
        with self.assertRaisesRegex(
            ...
            _ = CocoMetric(
                ann_file=self.ann_file_coco, format_only=True, outfile_prefix=None
            )
    ...
    def test_other_methods(self):
        ...
        metric_coco.process(data_batch, data_samples)
        # an extra sample
        eval_results = metric_coco.evaluate(size=len(self.topdown_data_coco) + 1)
    ...
    def test_format_only(self):
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test.keypoints.json")))
        ...
        with self.assertRaisesRegex(
            AssertionError, "Ground truth annotations are required for evaluation"
        ):
    ...
    def test_topdown_alignment(self):
        ...
        for ann in db["annotations"]:
            ...
            keypoints = np.array(ann["keypoints"], dtype=np.float32).reshape(1, -1, 3)
        ...
        self.assertTrue(
            osp.isfile(osp.join(self.tmp_dir.name, "test_align1.keypoints.json"))
        )
        ...
        self.assertTrue(
            osp.isfile(osp.join(self.tmp_dir.name, "test_align2.keypoints.json"))
        )
        ...
        for i, ann in enumerate(anns):
            ...
            gt_instances1 = {
                "bbox_scales": bbox_scales,
                "bboxes": bboxes,
                "bbox_scores": np.ones((1,), dtype=np.float32) * (1 - 2 * i / 100),
            }
            pred_instances1 = {
                "keypoints": keypoints[..., :2],
                "keypoint_scores": keypoints[..., -1],
        ...
        self.assertTrue(
            osp.isfile(osp.join(self.tmp_dir.name, "test_align3.keypoints.json"))
        )
    ...
    def test_topdown_evaluate(self):
        ...
        self.assertDictEqual(eval_results, self.target_coco)
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test1.keypoints.json")))
        ...
        self.assertDictEqual(eval_results, self.target_coco)
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test2.keypoints.json")))
        ...
        self.assertDictEqual(eval_results, self.target_coco)
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test3.keypoints.json")))
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test4.gt.json")))
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test4.keypoints.json")))
        ...
        eval_results = metric_crowdpose.evaluate(size=len(self.topdown_data_crowdpose))
        self.assertDictEqual(eval_results, self.target_crowdpose)
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test5.keypoints.json")))
        ...
        self.assertDictEqual(eval_results, self.target_crowdpose)
        # test whether convert the annotation to COCO format
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test6.gt.json")))
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test6.keypoints.json")))

        # case 7: test AP10k dataset
        metric_ap10k = CocoMetric(
            ann_file=self.ann_file_ap10k, outfile_prefix=f"{self.tmp_dir.name}/test7"
        )
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test7.keypoints.json")))
        ...
        for key in self.target_ap10k:
            self.assertAlmostEqual(eval_results[key], self.target_ap10k[key])
        # test whether convert the annotation to COCO format
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test8.gt.json")))
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test8.keypoints.json")))
    ...
    def test_gt_converter(self):
        ...
        self.assertTrue(
            osp.isfile(osp.join(self.tmp_dir.name, "test_convert.keypoints.json"))
        )
