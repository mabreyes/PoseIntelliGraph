...
class TestPoseTrack18Metric(TestCase):
    def setUp(self):
        ...
        self.ann_file = (
            "tests/data/posetrack18/annotations/" "test_posetrack18_val.json"
        )
        posetrack18_meta_info = dict(from_file="configs/_base_/datasets/posetrack18.py")
        self.posetrack18_dataset_meta = parse_pose_metainfo(posetrack18_meta_info)
    ...
    def test_bottomup_evaluate(self):
        ...
        eval_results = posetrack18_metric.evaluate(size=len(self.bottomup_data))
        self.assertDictEqual(eval_results, self.target)
        self.assertTrue(
            osp.isfile(osp.join(self.tmp_dir.name, "009473_mpii_test.json"))
        )
    ...
    def test_other_methods(self):
        ...
        posetrack18_metric.process(data_batch, data_samples)
        # an extra sample
        eval_results = posetrack18_metric.evaluate(size=len(self.topdown_data) + 1)
        self.assertDictEqual(eval_results, self.target)
    ...
    def test_topdown_alignment(self):
        ...
        for ann in self.db["annotations"]:
            ...
            keypoints = np.array(ann["keypoints"], dtype=np.float32).reshape(1, 17, 3)
