...
class TestYOLOXPoseAnnotationProcessor(TestCase):
    def test_encode(self):
        ...
        keypoints = np.array([[[0, 1], [2, 6], [4, 5]], [[5, 6], [7, 8], [8, 9]]])
        keypoints_visible = np.array([[1, 1, 0], [1, 0, 1]])
        bbox = np.array([[0, 1, 3, 4], [1, 2, 5, 6]])
        category_id = [1, 2]

        encoded = processor.encode(keypoints, keypoints_visible, bbox, category_id)

        self.assertTrue("bbox" in encoded)
        self.assertTrue("bbox_labels" in encoded)
        self.assertTrue(
            np.array_equal(
                encoded["bbox"], np.array([[0.0, 1.0, 3.0, 6.0], [1.0, 2.0, 8.0, 9.0]])
            )
        )
        self.assertTrue(np.array_equal(encoded["bbox_labels"], np.array([0, 1])))
