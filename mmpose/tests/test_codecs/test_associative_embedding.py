...
class TestAssociativeEmbedding(TestCase):
    ...
    def _get_tags(
        self, heatmaps, keypoint_indices, tag_per_keypoint: bool, tag_dim: int = 1
    ):
    ...
    def _sort_preds(self, keypoints_pred, scores_pred, keypoints_gt):
        ...
        costs = np.linalg.norm(
            keypoints_gt[None] - keypoints_pred[:, None], ord=2, axis=3
        ).mean(axis=2)
    ...
    def test_decode(self):
        ...
        keypoints, scores = self._sort_preds(
            batch_keypoints[0], batch_keypoint_scores[0], data["keypoints"]
        )
