...
class TestEDPoseLabel(TestCase):
    def setUp(self):
        ...
        self.keypoints = np.array([[[100, 50], [200, 50]], [[300, 400], [100, 200]]])
    ...
    def test_encode(self):
        ...
        expected_keypoints = self.keypoints / np.array(self.img_shape, dtype=np.float32)
        np.testing.assert_array_almost_equal(
            encoded_data["keypoints"], expected_keypoints
        )

        # Check if the area was normalized correctly
        expected_area = self.area / float(self.img_shape[0] * self.img_shape[1])
        np.testing.assert_array_almost_equal(encoded_data["area"], expected_area)
    ...
    def test_decode(self):
        ...
        pred_keypoints = np.array(
            [[0.2, 0.3, 1, 0.3, 0.4, 1], [0.6, 0.7, 1, 0.7, 0.8, 1]]
        )
        ...
        self.assertEqual(
            keypoints.shape, (self.encoder.num_select, self.encoder.num_keypoints, 2)
        )
        self.assertEqual(
            scores.shape, (self.encoder.num_select, self.encoder.num_keypoints)
        )
