...
class TestHand3DHeatmap(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.data = dict(
            keypoints=keypoints, keypoints_visible=keypoints_visible, heatmaps=heatmaps
        )
    ...
    def test_decode(self):
        ...
        keypoints, scores, _, _ = codec.decode(heatmaps, np.ones((1,)), np.ones((1, 2)))
    ...
    def test_cicular_verification(self):
        ...
        _keypoints, _, _, _ = codec.decode(
            encoded["heatmaps"].reshape(42, 64, 64, 64), np.ones((1,)), np.ones((1, 2))
        )

        self.assertTrue(np.allclose(keypoints[..., :2], _keypoints[..., :2], atol=5.0))
