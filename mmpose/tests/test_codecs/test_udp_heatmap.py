...
class TestUDPHeatmap(TestCase):
    ...
    def test_encode(self):
        ...
        for name, cfg in self.configs:
            ...
            self.assertEqual(
                encoded["heatmaps"].shape,
                (channel_per_kpt * 17, 64, 48),
                f'Failed case: "{name}"',
            )
            self.assertEqual(
                encoded["keypoint_weights"].shape, (1, 17)
            ), f'Failed case: "{name}"'
    ...
    def test_decode(self):
        for name, cfg in self.configs:
            ...
            heatmaps = np.random.rand(channel_per_kpt * 17, 64, 48).astype(np.float32)
    ...
    def test_cicular_verification(self):
        ...
        for name, cfg in self.configs:
            ...
            self.assertTrue(
                np.allclose(keypoints, _keypoints, atol=10.0),
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "invalid `heatmap_type`"):
            _ = UDPHeatmap(
                input_size=(192, 256), heatmap_size=(48, 64), heatmap_type="invalid"
            )
        ...
        with self.assertRaisesRegex(AssertionError, "only support single-instance"):
