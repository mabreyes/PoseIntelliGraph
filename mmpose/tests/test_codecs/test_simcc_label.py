...
class TestSimCCLabel(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.data = dict(keypoints=keypoints, keypoints_visible=keypoints_visible)
    ...
    def test_encode(self):
        ...
        for name, cfg in self.configs:
            ...
            self.assertEqual(
                encoded["keypoint_x_labels"].shape,
                (1, 17, int(192 * codec.simcc_split_ratio)),
                f'Failed case: "{name}"',
            )
            self.assertEqual(
                encoded["keypoint_y_labels"].shape,
                (1, 17, int(256 * codec.simcc_split_ratio)),
                f'Failed case: "{name}"',
            )
            self.assertEqual(
                encoded["keypoint_weights"].shape, (1, 17), f'Failed case: "{name}"'
            )
    ...
    def test_cicular_verification(self):
        ...
        for name, cfg in self.configs:
            ...
            self.assertTrue(
                np.allclose(keypoints, _keypoints, atol=5.0), f'Failed case: "{name}"'
            )
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "got invalid `smoothing_type`"):
        ...
        with self.assertRaisesRegex(ValueError, "`label_smooth_weight` should be"):
            _ = KEYPOINT_CODECS.build(cfg)
        ...
        with self.assertRaisesRegex(ValueError, "is only used for `standard` mode."):
            _ = KEYPOINT_CODECS.build(cfg)
