...
class TestVideoPoseLifting(TestCase):
    ...
    def build_pose_lifting_label(self, **kwargs):
        cfg = dict(type="VideoPoseLifting", num_keypoints=17, reshape_keypoints=False)
        cfg.update(kwargs)
    ...
    def test_encode(self):
        ...
        self.assertEqual(
            encoded["lifting_target_weight"].shape,
            (
                1,
                17,
            ),
        ...
        self.assertEqual(
            encoded["trajectory_weights"].shape,
            (
                1,
                17,
            ),
        ...
        # test removing root
        codec = self.build_pose_lifting_label(remove_root=True, save_index=True)
        encoded = codec.encode(
            keypoints,
            keypoints_visible,
            lifting_target,
        ...
        self.assertTrue(
            "target_root_removed" in encoded and "target_root_index" in encoded
        )
        self.assertEqual(
            encoded["lifting_target_weight"].shape,
            (
        ...
        self.assertTrue(
            np.allclose(
                camera_param["f"] / scale, encoded["camera_param"]["f"], atol=4.0
            )
        )
    ...
    def test_cicular_verification(self):
        ...
        _keypoints, _ = codec.decode(
            encoded["lifting_target_label"], target_root=lifting_target[..., 0, :]
        )

        self.assertTrue(np.allclose(lifting_target, _keypoints, atol=5.0))
