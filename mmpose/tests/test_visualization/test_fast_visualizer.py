...
class TestFastVisualizer(TestCase):
    def setUp(self):
        self.metainfo = {
            "keypoint_id2name": {0: "nose", 1: "left_eye", 2: "right_eye"},
            "keypoint_name2id": {"nose": 0, "left_eye": 1, "right_eye": 2},
            "keypoint_colors": np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255]]),
    ...
    def test_init(self):
        ...
        self.assertEqual(
            self.visualizer.keypoint_id2name, self.metainfo["keypoint_id2name"]
        )
        self.assertEqual(
            self.visualizer.keypoint_name2id, self.metainfo["keypoint_name2id"]
        )
        np.testing.assert_array_equal(
            self.visualizer.keypoint_colors, self.metainfo["keypoint_colors"]
        )
        self.assertEqual(
            self.visualizer.skeleton_links, self.metainfo["skeleton_links"]
        )
        np.testing.assert_array_equal(
            self.visualizer.skeleton_link_colors, self.metainfo["skeleton_link_colors"]
        )

    def test_draw_pose(self):
        img = np.zeros((480, 640, 3), dtype=np.uint8)
        instances = type("Instances", (object,), {})()
        instances.keypoints = np.array(
            [[[100, 100], [200, 200], [300, 300]]], dtype=np.float32
        )
        instances.keypoint_scores = np.array([[0.5, 0.5, 0.5]], dtype=np.float32)
