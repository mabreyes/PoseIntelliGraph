...
# codec settings
codec = dict(type="DecoupledHeatmap", input_size=(512, 512), heatmap_size=(128, 128))
...
model = dict(
    ...
    head=dict(
        ...
        contrastive_loss=dict(type="InfoNCELoss", temperature=0.05, loss_weight=1.0),
        decoder=codec,
    ),
    train_cfg=dict(max_train_instances=200),
    test_cfg=dict(
        multiscale_test=False, flip_test=True, shift_heatmap=False, align_corners=False
    ),
)
