...
class VideoProcessor:
    ...
    def pose_estimator(self) -> Pose2DInferencer:
        ...
        if self.category not in self._pose_estimator:
            ...
            self._pose_estimator[self.category].model.test_cfg["flip_test"] = False
    ...
    def get_keypoints_from_frame(self, image: np.ndarray) -> np.ndarray:
        ...
        det_results = self.pose_estimator.detector(image, return_datasamples=True)[
            "predictions"
        ]
        ...
        data = self.pose_estimator.collate_fn([self.pose_estimator.pipeline(data_info)])
        ...
        pred_instances = self.pose_estimator.model.head.predict(
            feats, data["data_samples"], test_cfg=self.pose_estimator.model.test_cfg
        )[0]
    ...
    def get_keypoints_from_video(self, video: str) -> np.ndarray:
        ...
        assert abs(video_reader.fps - 30) < 0.1, (
            f"only support videos with "
    ...
    @torch.no_grad()
    def run(self, tch_video: str, stu_video: str, output_file: Optional[str] = None):
        # extract human poses
        ...
        return self.generate_output_video(
            tch_video, stu_video, output_file, tch_kpts, stu_kpts, piece_info
        )
    ...
    def generate_output_video(
        ...
        for i in track_iter_progress(range(piece_info["length"])):
            ...
            stu_kpt = get_smoothed_kpt(stu_kpts, piece_info["stu_start"] + i, 5)
            tch_kpt = get_smoothed_kpt(tch_kpts, piece_info["tch_start"] + i, 5)
            ...
            stu_inst = InstanceData(
                keypoints=stu_kpt[None, :, :2], keypoint_scores=stu_kpt[None, :, 2]
            )
            tch_inst = InstanceData(
                keypoints=tch_kpt[None, :, :2], keypoint_scores=tch_kpt[None, :, 2]
            )
            ...
            out_img = blend_images(stu_out_img, tch_out_img, blend_ratios=(1, 0.3))
            ...
            if video_writer is None:
                video_writer = cv2.VideoWriter(
                    ...
                    30,
                    (concatenated_image.shape[1], concatenated_image.shape[0]),
                )
