...
class BBoxKeypoints(HorizontalBoxes):
    ...
    def __init__(
        ...
        super().__init__(
            data=data, dtype=dtype, device=device, clone=clone, in_mode=in_mode
        )
    ...
    def project_(self, homography_matrix: Union[Tensor, np.ndarray]) -> None:
        ...
        keypoints = torch.cat(
            [self.keypoints, self.keypoints.new_ones(*self.keypoints.shape[:-1], 1)],
            dim=-1,
        )
