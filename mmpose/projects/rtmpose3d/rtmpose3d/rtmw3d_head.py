...
class RTMW3DHead(BaseHead):
    ...
    def __init__(
        ...
        self.mlp2 = nn.Sequential(
            ...
            nn.Linear(flatten_dims * ps**2, gau_cfg["hidden_dims"] // 2, bias=False),
        )
    ...
    def forward(self, feats: Tuple[Tensor, Tensor]) -> Tuple[Tensor, Tensor, Tensor]:
        """Forward the network.
    ...
    def decode(self, batch_outputs: Union[Tensor, Tuple[Tensor]]) -> InstanceList:
        ...
        preds = []
        for keypoints, keypoints_simcc, scores in zip(
            batch_keypoints, batch_keypoints_simcc, batch_scores
        ):
            pred = InstanceData(
                keypoints=keypoints,
    ...
    def loss(
        ...
        gt_x = torch.cat(
            [d.gt_instance_labels.keypoint_x_labels for d in batch_data_samples], dim=0
        )
        gt_y = torch.cat(
            [d.gt_instance_labels.keypoint_y_labels for d in batch_data_samples], dim=0
        )
        gt_z = torch.cat(
            [d.gt_instance_labels.keypoint_z_labels for d in batch_data_samples], dim=0
        )
        ...
        for i, loss_ in enumerate(self.loss_module):
            if loss_.loss_name in ["loss_bone", "loss_mpjpe"]:
                pred_coords = get_3d_coord(pred_x, pred_y, pred_z, with_z_labels)
...
def get_3d_coord(simcc_x, simcc_y, simcc_z, with_z_labels):
    ...
    locs = torch.stack((x_locs, y_locs, z_locs), dim=-1).to(simcc_x).reshape(N, K, 3)
    return locs
