    ...
    def __call__(self, oriImg):
        ...
        for part in range(18):
            ...
            peaks_with_score_and_id = [
                peaks_with_score[idx] + (peak_id[idx],) for idx in range(len(peak_id))
            ]
        ...
        for k in range(len(mapIdx)):
            ...
            if nA != 0 and nB != 0:
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        vec_x = np.array(
                            [
                                score_mid[
                                    int(round(startend[midpoint_index][1])),
                                    int(round(startend[midpoint_index][0])),
                                    0,
                                ]
                                for midpoint_index in range(len(startend))
                            ]
                        )
                        vec_y = np.array(
                            [
                ...
                for c_idx in range(len(connection_candidate)):
                    i, j, s = connection_candidate[c_idx][:3]
        ...
        for k in range(len(mapIdx)):
            if k not in special_k:
                ...
                for n_idx in range(len(connection_all[k])):  # = 1:size(temp,1)
                    found = 0
                    subset_idx = [-1, -1]
                    for j in range(len(subset)):  # 1:size(subset,1):
                        if (
                            subset[j][indexA] == partAs[n_idx]
                            or subset[j][indexB] == partBs[n_idx]
                        ):
                            subset_idx[found] = j
                            found += 1
                    ...
                    if found == 1:
                        j = subset_idx[0]
                        if subset[j][indexB] != partBs[n_idx]:
                            subset[j][indexB] = partBs[n_idx]
                            subset[j][-1] += 1
                            subset[j][-2] += (
                                candidate[partBs[n_idx].astype(int), 2]
                                + connection_all[k][n_idx][2]
                            )
                        ...
                        if len(np.nonzero(membership == 2)[0]) == 0:  # merge
                            ...
                            subset[j1][-2] += connection_all[k][n_idx][2]
                            subset = np.delete(subset, j2, 0)
                        else:  # as like found == 1
                            subset[j1][indexB] = partBs[n_idx]
                            subset[j1][-1] += 1
                            subset[j1][-2] += (
                                candidate[partBs[n_idx].astype(int), 2]
                                + connection_all[k][n_idx][2]
                    ...
                    # if find no partA in the subset, create a new subset
                        ...
                        row = -1 * np.ones(20)
                        row[indexA] = partAs[n_idx]
                        row[indexB] = partBs[n_idx]
                        row[-1] = 2
                        row[-2] = (
                            sum(candidate[connection_all[k][n_idx, :2].astype(int), 2])
                            + connection_all[k][n_idx][2]
                        )
        ...
        for idx in range(len(subset)):
            if subset[idx][-1] < 4 or subset[idx][-2] / subset[idx][-1] < 0.4:
